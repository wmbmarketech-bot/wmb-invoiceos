...
function Sidebar({page,setPage,settings,isDark,onToggle}) {
  ...
  return (
    <div style={{width:exp?220:68,transition:"width 0.3s cubic-bezier(0.4,0,0.2,1)",
        ...
        flexShrink:0,overflow:"visible",zIndex:10,position:"relative",
      ...
      <button onClick={()=>setExp(e=>!e)}
        ...
        style={{position:"absolute",top:18,right:-14,width:28,height:28,borderRadius:"50%",
          border:`1px solid ${th.sideBdr}`,
          background:th.isDark?"rgba(10,20,50,0.9)":"rgba(255,255,255,0.95)",
          color:th.textSub,cursor:"pointer",zIndex:30,
          display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,
          boxShadow:th.isDark?"0 2px 8px rgba(0,0,0,0.4)":"0 2px 8px rgba(0,80,180,0.15)",
          backdropFilter:"blur(8px)",transition:"all 0.2s"}}
        onMouseEnter={e=>{e.currentTarget.style.color=th.accent;e.currentTarget.style.borderColor=th.accent;e.currentTarget.style.transform="scale(1.1)";}}
        onMouseLeave={e=>{e.currentTarget.style.color=th.textSub;e.currentTarget.style.borderColor=th.sideBdr;e.currentTarget.style.transform="scale(1)";}}>
        {exp?"‹":"›"}
      </button>
...
function InvoicePreview({invoice,companies}) {
  ...
  return <div style={{background:"#fff",borderRadius:14,overflow:"hidden",boxShadow:"0 4px 24px rgba(0,0,0,0.1)"}}>
    ...
    <div style={{padding:"22px 36px 28px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 18px",background:"linear-gradient(135deg,#0a1628,#1e3a5f)",borderRadius:10,marginBottom:18,WebkitPrintColorAdjust:"exact",printColorAdjust:"exact"}}>
        <div>
...
function IncomePage({invoices,setInvoices,companies,services,settings,setSettings}) {
  ...
  const openNew = () => {
    // Don't bump sequence yet — assign number only when actually saved
    setEditing({id:"__pending__",companyId:companies[0]?.id||"",client:{name:"",email:"",vat:"",address:""},
  ...
  const saveInv = inv => {
    let finalInv = inv;
    if(inv.id==="__pending__"||inv._isNew) {
      const [id,ns]=bumpNext(settings);
      setSettings(ns);
      finalInv = {...inv, id};
    }
    setInvoices(p=>{const i=p.findIndex(x=>x.id===finalInv.id);return i>=0?p.map((x,j)=>j===i?finalInv:x):[...p,finalInv]});
    setFormOpen(false);
  };
  ...
  const filtered = useMemo(()=>invoices.filter(inv=>{
    ...
    // Sort by invoice number descending (INV-0005 > INV-0004)
  ...
  return <>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
        <div>
          ...
          <div style={{fontSize:12,color:th.textSub,marginTop:4}}>
            Next: <span style={{color:th.accent,fontFamily:MONO,fontWeight:600}}>{peekNext(settings)}</span>
            {invoices.length>0&&<span style={{fontSize:11,color:th.textFaint,marginLeft:12}}>
              Latest issued: <span style={{fontFamily:MONO,color:th.textSub}}>{invoices.reduce((a,b)=>(parseInt(a.id.replace(/[^0-9]/g,""))||0)>(parseInt(b.id.replace(/[^0-9]/g,""))||0)?a:b,invoices[0])?.id||"—"}</span>
            </span>}
          </div>
        </div>
            ...
            return <tr key={inv.id}
              ...
              <td style={{padding:"12px 10px"}}>
                <div style={{display:"flex",gap:5,alignItems:"center"}}>
                  ...
                  <button title="Download PDF" onClick={e=>{e.stopPropagation();setPreviewing(inv);setPreviewOpen(true);}}
                  ...
                  <button title="Delete invoice" onClick={e=>{e.stopPropagation();if(confirm(`Delete ${inv.id}?\n\nThis cannot be undone. The invoice number will be permanently retired unless it is the latest.`)){
                    const allNums=invoices.map(x=>parseInt(x.id.replace(/[^0-9]/g,""))||0);
                    const maxNum=Math.max(...allNums);
                    const thisNum=parseInt(inv.id.replace(/[^0-9]/g,""))||0;
                    setInvoices(p=>p.filter(x=>x.id!==inv.id));
                    if(thisNum===maxNum) setSettings(p=>({...p,nextSeq:p.nextSeq-1}));
                    deleteFromDB("invoices",inv.id).catch(()=>{});
                  }}}
                    style={{padding:"5px 8px",borderRadius:7,border:`1px solid ${th.cardBdr}`,background:"transparent",
                      color:th.textSub,cursor:"pointer",fontSize:13,lineHeight:1,transition:"all 0.15s"}}
    ...
    <SlidePanel open={formOpen} onClose={()=>setFormOpen(false)}
      ...
      subtitle={editing?._isNew?`Will be assigned on save: ${peekNext(settings)}`:editing?.id}>
      ...
      {previewing&&<>
        <div style={{display:"flex",gap:8,marginBottom:16}}>
          ...
          <Btn sm variant="ghost" onClick={()=>{
            ...
            w.document.write('<html><head><title>'+(previewing?.id||'Invoice')+' — '+(companies.find(co=>co.id===previewing?.companyId)?.tradeName||'')+'</title><style>*{box-sizing:border-box;margin:0;padding:0;font-family:Inter,sans-serif;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}body{background:#fff;padding:0}@page{size:A4;margin:12mm;marks:none}@media print{html,body{margin:0;padding:0}*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}}</style></head><body>');
        ...
        <div id="invoice-preview-print"><InvoicePreview invoice={previewing} companies={companies}/></div>
      </>}
    </SlidePanel>
  </>;
}
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:16}}>
      {companies.map(co=><div key={co.id} onClick={()=>{setEdit({...co});setOpen(true);}}
        ...
        <div style={{padding:"18px 20px"}}>
          <div style={{fontSize:15,fontWeight:700,color:co.color||th.accent}}>{co.tradeName}</div>
          <div style={{fontSize:11,color:th.textSub,marginTop:3,fontWeight:500}}>{co.legalName}</div>
    ...
    {companies.map(co=>{
      ...
      return <div key={co.id} style={{marginBottom:24}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:10}}>
          {svcs.map(s=><div key={s.id}
            style={{...card(th,{padding:"14px 16px",transition:"all 0.15s",position:"relative"})}}
            onMouseEnter={e=>{e.currentTarget.style.borderColor=co.color||th.accent;e.currentTarget.style.background=th.cardHov;e.currentTarget.querySelector('.svc-del').style.opacity="1";}}
            onMouseLeave={e=>{e.currentTarget.style.borderColor=th.cardBdr;e.currentTarget.style.background=th.card;e.currentTarget.querySelector('.svc-del').style.opacity="0";}}>
            <button className="svc-del" onClick={e=>{e.stopPropagation();if(confirm(`Delete "${s.name}"?`)){del(s.id);deleteFromDB("services",s.id).catch(()=>{});}}}
              style={{position:"absolute",top:8,right:8,width:22,height:22,borderRadius:6,
                border:"none",background:th.redDim||`${th.red}18`,color:th.red,cursor:"pointer",
                fontSize:12,display:"flex",alignItems:"center",justifyContent:"center",
                opacity:0,transition:"opacity 0.15s"}}>×</button>
            <div style={{fontSize:13,fontWeight:500,color:th.text,marginBottom:5,cursor:"pointer",paddingRight:20}}
              onClick={()=>{setEdit({...s});setOpen(true);}}>{s.name}</div>
