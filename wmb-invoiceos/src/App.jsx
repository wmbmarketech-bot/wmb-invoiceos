...
const SEED = {
  companies:[
    {id:"c1",legalName:"WMB Marketech LTD",tradeName:"HealthTech Cyprus Expo",website:"healthtechcyprusexpo.com",address:"Limassol, Cyprus",vat:"60021843M",email:"info@healthtechcyprusexpo.com",phone:"+35795604079",reg:"HE408385",color:"#38bdf8",bank:{name:"Bank of Cyprus",iban:"CY08002001950000357038391815",swift:"BCYPCY2N",holder:"WMB MARKETECH LTD"}},
    {id:"c2",legalName:"WMB Marketech LTD",tradeName:"Kartatek Solutions",website:"kartatek.com",address:"Limassol, Cyprus",vat:"60021843M",email:"info@kartatek.com",phone:"+35795604079",reg:"HE408385",color:"#a78bfa",bank:{name:"Bank of Cyprus",iban:"CY08002001950000357038391815",swift:"BCYPCY2N",holder:"WMB MARKETECH LTD"}},
  ],
  services:[
...
const Pill = ({status}) => {
  ...
  return <span style={{display:"inline-flex",alignItems:"center",gap:5,padding:"3px 11px",borderRadius:20,
    fontSize:11,fontWeight:600,background:c.bg,color:c.text,border:`1px solid ${c.bdr}`,
...
function Sidebar({page,setPage,settings,isDark,onToggle}) {
  ...
  const th = useT();
  return (
    <div onMouseEnter={()=>setExp(true)} onMouseLeave={()=>setExp(false)}
      style={{width:exp?220:68,transition:"width 0.3s cubic-bezier(0.4,0,0.2,1)",
      ...
      {/* Logo */}
      <div style={{padding:"22px 16px 18px",borderBottom:`1px solid ${th.sideBdr}`,
        ...
        <div style={{opacity:exp?1:0,transition:"opacity 0.18s",whiteSpace:"nowrap",overflow:"hidden"}}>
...
function IncomePage({invoices,setInvoices,companies,services,settings,setSettings}) {
  ...
  const filtered = useMemo(()=>invoices.filter(inv=>{
    ...
    return true;
  }),[invoices,fCo,fSrc,fPay,fStat,search]);
  ...
  return <>
    ...
    <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
      <input placeholder="Search…" value={search} onChange={e=>setSearch(e.target.value)} style={{...filterSel,width:180}}/>
      {[
      ...
      <table style={{width:"100%",borderCollapse:"collapse"}}>
        <thead style={{position:"sticky",top:0,zIndex:5}}>
          <tr style={{background:th.isDark?"rgba(255,255,255,0.04)":"rgba(255,255,255,0.7)",borderBottom:`1px solid ${th.cardBdr}`,backdropFilter:"blur(8px)"}}>
            {["Invoice","Client","Entity","Source","Method","Total","Status",""].map(h=>
        ...
        <tbody>
          ...
          {filtered.map(inv=>{
            ...
            return <tr key={inv.id}
              ...
              <td style={{padding:"12px 14px",fontSize:11,color:th.textSub}}>{inv.source}</td>
  ...
  return <>
    ...
    <SlidePanel open={open} onClose={()=>setOpen(false)} title={editing?._isNew?"New Entity":"Edit Entity"}>
      {editing&&<div style={{display:"flex",flexDirection:"column",gap:12}}>
        ...
        <div><Lbl>Brand color</Lbl><div style={{display:"flex",gap:10,marginTop:4}}>{COLS.map(c=><button key={c} onClick={()=>setEdit(p=>({...p,color:c}))} style={{width:28,height:28,borderRadius:"50%",background:c,border:editing.color===c?"3px solid #fff":"3px solid transparent",cursor:"pointer",boxShadow:editing.color===c?`0 0 12px ${c}`:"none",transition:"all 0.15s"}}/>)}</div></div>
            ...
            <div>
              ...
              <input type="email" value={email} onChange={e=>setEmail(e.target.value)} required
                ...
                style={{width:"100%",background:"rgba(255,255,255,0.7)",border:"1px solid rgba(180,200,240,0.6)",
                  color:"#0f172a",borderRadius:12,padding:"11px 14px",fontFamily:"'Outfit',sans-serif",
...
export default function Root() {
  ...
  if (session === undefined) return (
    <div style={{display:"flex",height:"100vh",alignItems:"center",justifyContent:"center",
      background:"linear-gradient(135deg,#dbeafe,#ede9fe,#fce7f3)",fontFamily:"'Outfit',sans-serif"}}>
      <div style={{textAlign:"center"}}>
        <div style={{fontSize:32,marginBottom:10}}>⚡</div>
        <div style={{fontSize:15,fontWeight:600,color:"#0284c7"}}>Loading…</div>
      </div>
    </div>
  );
